...
def dts_metadata():
    return (
        flatten_df(dlt.readStream("bronze"))
        .withColumn("unique_well_id", F.regexp_replace(F.col("well_path"), root_to_remove, ""))
        .withColumn("unique_well_id", F.regexp_replace(F.col("unique_well_id"), "Halliburton/", ""))
        .withColumn("unique_well_id", F.regexp_replace(F.col("unique_well_id"), "Petrospec/", ""))
...
dlt.create_streaming_table("dts_metadata_scd")

dlt.apply_changes(
  target = "dts_metadata_scd",
  source = "dts_metadata",
  keys = ["wellSet_well_wellboreSet_wellbore_dtsMeasurementSet_dtsMeasurement__uid"],
